...
| `rerank_enabled` | bool | `true` | Rescore the top `rerank_depth` direct results with a cross-encoder after merge, before graph expansion. Requires the model on disk; absent model → stage skipped. |
