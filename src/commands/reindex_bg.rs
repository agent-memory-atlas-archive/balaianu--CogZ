...
/// Run the background reindex process. Opens the DB, runs
/// `reindex_code` (git diff based), flags stale knowledge, and
/// spawns `embed-bg` for synced entities.
pub fn run_reindex_bg(repo: &Path, db_path: &Path) -> anyhow::Result<()> {
    ...
    // Run the incremental reindex (git diff + processing + baseline).
    ...
    // Defer embedding to the background process.
    if !result.synced_entity_ids.is_empty() {
        spawn_embed_bg(&storage, &config, &result.synced_entity_ids);
    }
